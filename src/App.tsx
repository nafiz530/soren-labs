...
function PhysicsLab({id}:{id:string}){const[a,setA]=useState(id==="waves"?35:id==="ohm"?6:id==="optics"?80:30),[b,setB]=useState(id==="waves"?2:id==="ohm"?10:id==="optics"?80:5),[c,setC]=useState(id==="newton"?4:5);
 const ref=useCanvas((ctx,w,h,t)=>{Grid({ctx,w,h});ctx.strokeStyle="#5662d9";ctx.lineWidth=4;
  ...
  else if(id==="optics"){const cx=w/2,cy=h/2,focus=b;ctx.strokeStyle="#5662d9";ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(cx,25);ctx.lineTo(cx,h-25);ctx.stroke();ctx.fillStyle="#111";ctx.fillRect(Math.max(35,cx-a),cy-55,7,55);ctx.strokeStyle="#111";ctx.lineWidth=2;for(const yy of [cy-55,cy-25]){ctx.beginPath();ctx.moveTo(Math.max(42,cx-a),yy);ctx.lineTo(cx,yy);ctx.lineTo(cx+focus,cy);ctx.stroke()}ctx.fillStyle="#737c88";ctx.beginPath();ctx.arc(cx-focus,cy,5,0,7);ctx.arc(cx+focus,cy,5,0,7);ctx.fill()}
  else {for(let x=0;x<w;x+=2){const y=h/2+a*Math.sin(x*.012*b-t*3);x?ctx.lineTo(x,y):ctx.moveTo(x,y)}ctx.stroke()}
 },[id,a,b,c]);
 const labels=id==="newton"?["Force","Mass","Friction"]:id==="ohm"?["Voltage","Resistance","Temperature"]:id==="optics"?["Object distance","Focal length","Ray height"]:["Amplitude","Frequency","Wave speed"];
 return <Stage readout={id==="newton"?<>Acceleration <b>{Math.max(0,(a-c)/Math.max(b,1)).toFixed(2)} m/s²</b> · F = ma</>:id==="ohm"?<>Current <b>{(a/Math.max(b,.1)).toFixed(2)} A</b> · Power <b>{(a*a/Math.max(b,.1)).toFixed(1)} W</b> · V = IR</>:id==="optics"?<>Lens focal length <b>{b}</b> · Move the object to explore image formation.</>:<>Amplitude <b>{a}</b> · Frequency <b>{b}</b> Hz · Wave period <b>{(1/Math.max(b,.1)).toFixed(2)} s</b></>} controls={<><Range label={labels[0]} value={a} min={id==="newton"?0:id==="ohm"?1:id==="optics"?30:5} max={id==="newton"?60:id==="ohm"?12:id==="optics"?180:70} onChange={setA} unit={id==="ohm"?" V":id==="newton"?" N":""}/><Range label={labels[1]} value={b} min={id==="newton"?1:id==="ohm"?1:id==="optics"?30:.5} max={id==="newton"?20:id==="ohm"?30:id==="optics"?140:5} step={id==="waves"?.1:1} onChange={setB} unit={id==="ohm"?" Ω":id==="newton"?" kg":id==="waves"?" Hz":""}/><Range label={labels[2]} value={c} min={0} max={id==="newton"?20:100} onChange={setC}/></>}><canvas ref={ref}/></Stage>
}
function ChemistryLab({id}:{id:string}){const[a,setA]=useState(8),[b,setB]=useState(16),[c,setC]=useState(50);
 ...
 return <Stage readout={id==="atom"?<>Atomic number <b>{a}</b> · Mass number <b>{b}</b> · Neutrons <b>{Math.max(0,b-a)}</b></>:id==="bonding"?<>{a} shared electron pair{a>1?"s":""} · Observe how bond order changes.</>:id==="ph"?<>pH <b>{clamp(7+(b-a)/15,0,14).toFixed(1)}</b> · {a===b?"Neutralisation point":"Adjust acid/base volumes."}</>:id==="reaction"?<>Relative rate <b>{((a/8)*(b/50)*(c/50)).toFixed(2)}×</b> · More frequent energetic collisions increase rate.</>:<>Applied condition <b>{c}%</b> · Observe ion movement.</>} controls={<><Range label={id==="atom"?"Atomic number":id==="bonding"?"Shared pairs":id==="ph"?"Acid volume":"Primary condition"} value={a} min={id==="atom"?1:1} max={id==="atom"?20:id==="bonding"?3:100} onChange={setA}/><Range label={id==="atom"?"Mass number":id==="ph"?"Base volume":"Secondary condition"} value={b} min={1} max={id==="atom"?40:100} onChange={setB}/><Range label={id==="atom"?"Extra condition":"Intensity"} value={c} min={0} max={100} onChange={setC}/></>}><canvas ref={ref}/></Stage>
}
